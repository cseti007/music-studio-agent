...
PRESETS: dict[str, dict] = {
    ...
    "vocal_plate": {
        "description": "Small plate for vocal lead — 1.5 sec decay, classic studio plate sound",
        "notes": "HP at 300 Hz keeps the plate out of the vocal's chest tone. LP at 8 kHz controls the sibilance bouncing in the reverb. 10ms pre-delay keeps lyrics intelligible. Typical send level for lead vocal: 0.15-0.25 wet.",
        "room_size": 0.55,
        "damping": 0.35,
        "width": 1.0,
        "pre_delay_ms": 10.0,
    ...
    "vocal_chamber": {
        "description": "Small chamber for vocal — natural acoustic space, less artificial than plate",
        "notes": "Quick pre-delay (8 ms) and short decay (~0.6 s) — keeps the vocal feeling close while adding depth. Use as a primary vocal reverb for ballad / intimate styles. HP at 250 Hz softer than plate so a bit of chest tone bleeds in.",
        "room_size": 0.40,
        "damping": 0.50,
        "width": 0.90,
        "pre_delay_ms": 8.0,
    ...
    "vocal_hall_wide": {
        "description": "Wide hall for vocal — long decay, sense of size, modern pop / ballad",
        "notes": "Long pre-delay (60 ms) keeps the vocal articulation forward while the hall tail blooms behind it. ~2 s decay. Use as a secondary send alongside vocal_plate — typical level 0.10-0.15. HP 400 Hz keeps the bottom out of the hall.",
        "room_size": 0.80,
        "damping": 0.30,
        "width": 1.0,
        "pre_delay_ms": 60.0,
